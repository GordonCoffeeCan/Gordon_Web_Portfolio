...
function initPage3() {
	...
	loader.addEventListener("complete", handleCompletePage3);
	loader.loadManifest(lib_page3.properties.manifest);
}
function handleFileLoad(evt) {	
	if (evt.item.type == "image") { images[evt.item.id] = evt.result; }	
}
function handleCompletePage3(evt) {
	//This function is always called, irrespective of the content. You can use the variable "stage" after it is created in token create_stage.
	var queue = evt.target;
	var ssMetadata = lib_page3.ssMetadata;
	...
	fnStartAnimation = function() {
		createjs.Ticker.setFPS(lib_page3.properties.fps);
	...
	function makeResponsive(isResp, respDim, isScale, scaleType) {		
		...
		function resizeCanvas() {			
			var w = lib_page3.properties.width, h = lib_page3.properties.height;			
